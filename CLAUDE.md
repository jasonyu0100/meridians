...
A simulation engine for long-form reasoning. InkTide extracts and generates **World Views** — causally coherent, highly mutable, queryable knowledge structures over the three force fields (fate, world, system). Any coherent text describes a world view: *Harry Potter and the Sorcerer's Stone* has one, a research paper has one, a wargame brief has one. World view and narrative are interchangeable terms here — narrative is the canonical case study, world view is the underlying abstraction. The engine spans **fiction**, **non-fiction** (research, essay, argument), and **real-world simulation** (scenario forecasts, alternate-history modeling, strategic timelines). Pipeline: **priors → world view → timelines → forecast**. Feed the engine rich context, it extracts a typed knowledge graph, you branch alternative trajectories, and structural intelligence comes out. Next.js 16 + React 19 + TypeScript.

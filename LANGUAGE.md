...
## Vision thesis — Capture & Playback

**Vision thesis** — the bet the whole product rests on: **human vision is humanity's edge over AI.** Models scale prediction, language, search, and optimisation faster than any operator; what they don't originate is the act of *choosing which future to play toward, who gets a seat, and which moves are worth making*. Meridians is built to honour and compound that act. It is exercised in **two modes** — Capture and Playback — *two sides of one coin, run at two tempos*: **Capture is the asynchronous practice** (the slow side, compounds), **Playback the live practice** (the fast side, sells).
...
**Stream** — A single **perspective's** bearing on one open question, gathered over time. The per-seat unit of the belief layer: a Stream *is* a Thread (same named outcomes + a Stance of logits / volume / volatility, evolved by evidence) but **owned by one member against one perspective**, and its belief-log nodes are its **priors**. Many Streams can hold the *same* question — each a different seat's independently-seeded stance — forming a *local market* whose divergence is preserved, never averaged. Opened from a member's seeding intuition (prior #1), a Stream runs *open* while gathering, *committed* when folded into a Merge, and *closed* once the question settles. **Threads are the world view's belief over narrative questions; Streams are the parallel, perspective-scoped belief layer feeding the room.**
...
**Member** — A human participant in the room. Exactly one holds the **Game Master** role (the single master device); the rest hold seats. Carries a mobile number as the routing key for live (ngrok) and dark (WhatsApp) access.
...
**Conviction** — The first **rehearsal mode**: the War Room played as a card game on the substrate (specced in [CONCEPT.md](CONCEPT.md); *not yet built*). A game about *belief made visible* — seats spend scarce **conviction** to back what they believe about the near future, **show or hide** it to coordinate or mislead, and the engine plays the table's bets forward into a kept branch. It is the **fast twin of Capture**: same threads, stances, and resolver, run in a session instead of over weeks. Rehearsal is a **pluggable layer** over the shared resolver — other game formats can ride the same substrate later; Conviction is where it starts. *Capitalised "Conviction" is the game; lowercase "conviction" is the resource it runs on — priced by improbability (`−log p`), it banks but decays (`α = 0.9`).*

**Access model — Console / Live (ngrok) / Dark (WhatsApp)** — How a room runs across devices. The **console** is the laptop (the *master device*) that holds the substrate, drives the shared screen everyone reads, and is the **single source of truth** — no sync, no merge, no conflict. Non-GMs reach it two ways. **Live (ngrok):** when the GM hosts, an **ngrok tunnel** gives the local instance a public URL; the GM posts it to the WhatsApp group and non-GMs open it on phone or desktop. They get the **full Meridians interface** — same substrate, board, and live state — with **read-write** access (submit inputs, adjust stances, play moves, interact with the graph), functioning like a cloud-hosted app for the session; **PIN-gated**, with the GM keeping **elevated privileges**. Not a crippled mobile view — the real interface. **Dark (WhatsApp):** when the machine is off, the instance is dark (no interface); the WhatsApp group stays open as the **always-on capture layer** where members drop priors, observations, and belief shifts any time. **No server-side component** for either — ngrok is a third-party tunnel, WhatsApp is a normal group. *The console holds the truth; live you join it over the tunnel, dark you feed it through WhatsApp.*

**Game Master** — The single person who owns the master device and the room: keeps the data safe (the substrate is one encrypted `.meridian` file, backed up by the operator, optionally copied to an in-app online drive), convenes the sessions, refines the models, and holds the truth. May be a **volunteer inside the organization** or a **facilitator on our side**. The GM is the **curation-and-commit layer and the gatekeeper**: decides when the instance lives (hosts it, fires the ngrok tunnel, shares the URL), and which inputs graduate into the substrate — reviewing the WhatsApp capture inbox when the instance comes back online, curating, and committing what earns it. Holds **elevated privileges** while non-GMs have full read-write over the tunnel. Everyone else joins as a seat. *Local-first by default; data responsibility is the operator's.*
...
**Surfaces** — The four clusters the workspace is organised around, the general language users learn (the StageBar groups its ~29 view modes into these). **Vision** (the *Capture* cluster) — the room / perspective workspace: priors ingest (**Priors**), **Streams**, and commit **History** (Merges). **State** — the world view as state: the **Board** (map + location hierarchy), the graph domains (**World / System / Threads / Network**, each at *Scene / Arc / Full* scope), and the **Curriculum**. **Mind** — the stance and machinery: **Belief**, **Compass**, **Phase** (Mode Graph), **Decision** (Matrix), **Map**, and **Search**. **Channel** (the *Scene* cluster) — the move being authored: **Plan / Prose / Audio / Questions**. The console shows these; non-GMs reach the full set live over the ngrok tunnel, and capture async through WhatsApp when it's dark.
...
**Coverage** — Per-**member** spaced-repetition recall layered over the curriculum: who has been tested on what, and what is due. Turns *Onboarding decks* (passive playback) into *active recall* — the testing counterpart to the deck.
...
10. **Console / live (ngrok) / dark (WhatsApp) is the access language.** The console is the master device (the laptop that holds the substrate and drives the shared screen). **Live**, non-GMs reach the *full interface* over a PIN-gated **ngrok tunnel** (read-write; GM elevated). **Dark**, the **WhatsApp group** is the async capture layer; the GM curates and commits. There is **no server-side component** (ngrok = third-party tunnel, WhatsApp = normal group). In user-facing copy don't call the console "the server"; don't call the ngrok view a "controller" or a "crippled mobile view" — it's the real interface.
...
13. **The queue is now "Priors."** "Queue" / "Driver" survive only as code/UI labels inside the **Vision** (Capture) surface; in prose and vocabulary it's *Priors*.
...
18. **Conviction (capital) is the game; conviction (lowercase) is the resource.** Conviction is the *first* rehearsal mode, not the only one — rehearsal is a pluggable layer. Don't equate "Rehearsal" with "Conviction"; don't call conviction "the betting chips".
...
20. **Capture is the asynchronous practice; Playback is the live one.** Tempo is the clean split — Capture runs *async* (priors recorded over time, between sessions, no convening), Playback runs *live* (a synchronous, high-feedback session the room plays together). Don't blur it: don't call Capture "live" or Playback "asynchronous" at the practice level. (The access model's *live (ngrok) / dark (WhatsApp)* is a separate, device-reach layer — orthogonal to this practice-tempo split.)

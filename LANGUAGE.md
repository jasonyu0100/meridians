...
**Access model — Console / Live (ngrok) / Dark (WhatsApp + Slack)** — How a room runs across devices. The **console** is the laptop (the *master device*) that holds the substrate, drives the shared screen everyone reads, and is the **single source of truth** — no sync, no merge, no conflict. Non-GMs reach it two ways. **Live (ngrok):** when the GM hosts, an **ngrok tunnel** gives the local instance a public URL; the GM posts the link and non-GMs open it on phone or desktop. They get the **full Meridians interface** — same substrate, board, and live state — with **read-write** access (submit inputs, adjust stances, play moves, interact with the graph), functioning like a cloud-hosted app for the session; **PIN-gated**, with the GM keeping **elevated privileges**. Not a crippled mobile view — the real interface. **Dark:** when the machine is off, the instance is dark (no interface); the **always-on capture layer** stays open for members to drop priors, observations, and belief shifts any time. **WhatsApp and Slack are the two channels we primarily support** — WhatsApp for everyday rooms (direct, informal), Slack/Teams for businesses (already trusted). **No server-side component** for either — ngrok is a third-party tunnel; the channels are normal group chats. *The console holds the truth; live you join it over the tunnel, dark you feed it through WhatsApp or Slack.*
...
10. **Console / live (ngrok) / dark (WhatsApp + Slack) is the access language.** The console is the master device (the laptop that holds the substrate and drives the shared screen). **Live**, non-GMs reach the *full interface* over a PIN-gated **ngrok tunnel** (read-write; GM elevated). **Dark**, the async capture layer runs over the **two channels we primarily support — WhatsApp (everyday, informal) and Slack/Teams (businesses, trusted)**; the GM curates and commits. There is **no server-side component** (ngrok = third-party tunnel; the channels = normal group chats). In user-facing copy don't call the console "the server"; don't call the ngrok view a "controller" or a "crippled mobile view" — it's the real interface.

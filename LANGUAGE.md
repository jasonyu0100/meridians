...
**The loop — Capture, Rehearsal, Review** — Playing Meridians is one loop in three beats on a shared substrate (rule of three). **Capture** (formerly *Calibration*) assembles the priors and is **per-perspective by design**: every seat sharpens *its own* stance on *its own* open threads as priors accumulate. It is **not** belief-pooling — what is shared is the *board, not the belief* — which is exactly what you must not collapse when a seat models an adversary who should surprise you (its seat keeps deliberately hostile priors). Priors arrive two ways: **live**, each member tends their own seat through the full interface; **dark**, they drop observations into WhatsApp and the GM routes each into the seat it came from. A sparse seat just holds a wider, less-settled stance — never folded into another's. **Rehearsal** is the breadth game, the bulk of the War Room: only the moves the room *commits to or holds at high certainty* advance the simulation; the contested space is played forward over the **Compass** through board / card / information dynamics — a *compressed simulation of reality*, to *rehearse possible futures*. A committed stance is a *soft prior, not a constraint* — contested threads are protected and play-throughs carry a divergence directive, so Rehearsal explores the state space instead of re-enacting the prior. **Review** closes the loop: how the playthrough varied against the predicted **Compass** cohort (the divergence becomes the next thread to capture), a per-decision causal audit (**Butterfly**), and a generated **slide-deck** summary delivered to members' phones. Every play-through is a fork the substrate keeps and grades against what reality returns — the check that keeps the loop honest.
...
**Rehearsal** — The breadth game (the middle phase; see *The loop — Capture, Rehearsal, Review*) in session. The room plays the future forward — first along the compass's probabilistically ranked continuations, then along free-form branches the operators want to test on instinct. Every play-through is a fork the substrate keeps and grades against what reality returns. *The purpose of the play is to rehearse possible futures.*

**Review** — The third phase; the retrospective that closes the loop. Three things in one phase: (1) the **post-play audit** — how the room's forward play varied against the *predicted state space* (the Compass cohort the substrate softmax-ranked); the divergence is the signal, becoming the next thread to capture; (2) **Butterfly** — the per-decision causal audit of what reality did to each committed call; (3) **slide-deck delivery** — a generated summary of the session (decisions, resolved threads, what moved) pushed to members' phones, which doubles as onboarding/playback. Review is what turns Rehearsal into a high-feedback loop rather than a one-off. (Was *War Room Review*; Butterfly and deck delivery are now folded in.)

**Onboarding decks (playback)** — Generated **slide decks** over a slice of the substrate — a room's history and the key decisions that shaped it — that **update a newcomer's priors** when they join a team, move between teams, or are introduced to a new room. The lightweight onboarding surface (no session to convene), and it **regenerates as the substrate moves** so it stays current. This is the deck-delivery mechanism that **Review** ships to members' phones; it doubles as the **knowledge-transfer** surface. Butterfly verdicts and resolved threads make natural deck beats.

...
**Meridians is a rehearsal engine for the future** — an evolving game that codifies reality, letting teams operate in complex decision-making environments with the advantage of practice and foresight. Specifically: it lives in the unclaimed segment where *gaming, education, and strategy converge*. Not a strategy tool dressed up as a game; not a serious game pretending to be entertainment; not an educational platform with a strategy layer bolted on. An evolving game that adapts to its players' scenarios and turns their reality into playable worlds. A team knows it's prepared when its canonical models of reality carry high **Prior** scores — the present keeps landing on ground it has already walked; this human-up (not data-down) read of the future, the present, and the decision between them is the behaviour it sets out to change. The world view sharpens because the team keeps updating it; it stays alive because the team keeps playing it. *Maintenance is the practice, and the practice is the value.* When choosing vocabulary across docs, lean into the **gaming + education + strategy** triad — avoid framing that collapses Meridians into any single one of the three.
...
- **Capture** — the *slow, asynchronous* side. Capturing reality via **Priors**: each seat records its running stance *over time, no convening required* — observations dropped in as life happens, from each perspective's own vantage — tracked and calibrated until the room is a **rehearsal engine for the future** that compounds. The retention engine and the moat — the reason a team stays. Asynchronous by nature: it runs *between* sessions, on each member's own clock. (The prior-collection surface is **Priors**; the loop beat is **Capture**, below.)
...
**Influence** — The alluvial view of who is doing the pulling over time. Pick a source (Fate / World / System / Streams); each band is one container (a question, an entity, a rule), its width at every scene-bucket equal to the attention it drew there. Bands enter, swell, hand influence to one another, and resolve. **Unit** mode draws one band per container; **Type** mode re-groups the same flow by log kind. The literal picture of how things influence one another over a run.
...
**Stream** — A single **perspective's** bearing on one open **question of what to do**, gathered over time. The per-seat unit of the belief layer, and the key contrast with a Thread: **Streams are action-first.** Where a Thread tracks an *outcome* (will X happen? — a distribution over how a question resolves), a Stream holds an *open question of what to do* and tracks the perspective's leaning over the **candidate actions** it could take — its stance the calibration of turning thought into action. A Stream reuses the Thread stance machinery (named slots + a Stance of logits / volume / volatility, evolved by evidence), but each slot is a possible **move**, not a resolution; the result a move might produce is downstream and never definite. It is **owned by one member against one perspective**, and its belief-log nodes are its **priors** — the perspective's stream of consciousness as it deliberates. Many Streams can hold the *same* question — each a different seat's independently-seeded leaning — forming a *local market* whose divergence is preserved, never averaged. Opened from a member's seeding intuition (prior #1), a Stream runs *open* while the deliberation continues, *committed* when folded into a Merge, and *closed* once the move is settled. **Threads are the world view's belief over narrative outcomes; Streams are the parallel, perspective-scoped layer calibrating each seat's intended actions, feeding the room.** *The Stream is the **transducer that makes the qualitative quantitative**: a prose prior is scored into a shift of intent, and a chain of priors becomes a tracked leaning — distribution over moves, margin, uncertainty, volatility — settling over time. That one move is what makes the rest measurable: a stream is a decision under deliberation (which move holds, decays, gets overturned); cross-seat stance distance reads as **alignment** one way and **diversity** the other. See **Believability** below.*
...
**Conviction** — The **live practice** (the fast side of the Vision thesis; formerly "Playback"): the War Room convened and played in real time, where judgement is battle-tested against AI-simulated teams ([CONCEPT.md](CONCEPT.md); **shipped** — the Rounds variant in computer mode; the player loop is **Read → Write → Play** with generation calls between phases — (Perspective Gen) → Read → Write → (Stream & Intuition Gen) → Play → (Arc Gen); remote/Showdown deferred). A game about *belief made visible* — seats spend scarce **conviction** to back what they believe about the near future, **show or hide** it to coordinate or mislead, and the engine plays the table's bets forward into a kept branch. It is the **fast twin of Capture**: same threads, stances, and resolver, run in a session instead of over weeks. It's played as the **Rehearsal** card game; rehearsal is a **pluggable layer** over the shared resolver, so other live formats can ride the same substrate later. *Capitalised "Conviction" is the live practice / game; lowercase "conviction" is the resource it runs on — priced by improbability (`−ln p`, scale 20), it banks but decays (`α = 5/6 ≈ 0.83`, hoard ceiling 150).* **A laptop / desktop game** — reading the perspective and writing legible priors are the cornerstones of orienting and making a play, so it wants screen real estate and a keyboard; mobile is not a play surface (only the deferred, lightweight `remote` controller is phone-shaped).
...
- **Reality-anchored** — stakes pooled on real-world questions, with prize pools.
...
**Recall** — When a situation lands, the engine asks *have we been here before?* — matching it against the cumulative branch history by meaning (cosine over scene **summary embeddings**, see **Embeddings**), weighted toward recent play. Grounded in recognition-primed decision science (Klein): under pressure experts recognise and retrieve a rehearsed move rather than weigh options. A situation counts as **recalled** only if the match clears a similarity threshold **and** the matched branch was **room-played, fresh, and carries a committed response** — cosine alone is geometric approximation, not recognition. High recall infers **preparedness**: the room has met this and knows the move. (Also the IR sense — of the relevant cases, how many we retrieved.)

**Prior Knowledge & Foresight** — **The two North Stars.** We don't sell prediction; we sell **rehearsal at scale**, and these measure the readiness it buys, scene by scene. Both read the one cosine matrix over scene **summary embeddings** pooled across *all branches*: split by play order, earlier scenes feed **Prior Knowledge** (backward — *have we been here before?*), later scenes feed **Foresight** (forward — *do we already foresee what's coming?*). Each similarity is passed through a steep logistic centred on a match boundary (`s₀ = 0.80, κ = 22`) and the strength is the **mean of the top-K** (`K = 5`) — so a 0.90 match counts ~9× a 0.70, five strong matches sway far more than one, and mediocrity collapses to zero. A backward match on *another* branch — a future the room actually rehearsed — is weighted up (`×1.4`). Advisory and continuous, computed from the simulation; their reality-anchored proof is **Recall Share** (below). *The geometry advises; reality scores.*

**Experience** — The additive accrual of cross-branch rehearsal value over a branch's scenes → a progression **Level**. Deeper, more widely played branches carry more rehearsed moments and level up: preparedness earned by play, not by volume of generated-but-unplayed branches.

**Recall Share** — The **reality-anchored** metric: the external proof of Prior Knowledge & Foresight, anchored in reality rather than the simulation. Of the **consequential events that actually landed** this period, the share the room had already rehearsed (recalled per **Recall**, above) — *"eleven hit the desk, nine were recalled."* Because the events are real and checkable, an outsider can verify it; the practice never grades itself. Reported per **branch** and per **arc**, **raw** and **recency-weighted**, 0–100. **Wider play and regular capture are the INPUT** (the way training is the input to match performance), never the score itself. *Recognition is auditable now; accuracy (calibration on confirmed thread resolutions, Brier) is the slower second layer — see **Believability**.* Two scorings toggle in the top bar: **Narrative scoring** (the three forces) and **Experience scoring** (Prior Knowledge & Foresight) — both supported, neither replaces the other.

**Believability** — Weighting an idea by the track record of the seat behind it, not its seniority — an **idea meritocracy** you can audit ([Dalio's Principles](https://www.simonandschuster.com/books/Principles/Ray-Dalio/9781501124020) is the cultural antecedent — the track record it asserted but never had a number for). Two claims, kept distinct so neither overreaches: **recognition is auditable now** — a seat's committed stance is frozen before the outcome, its played branches stay on file with reasoning attached, and Recall shows when the room had met a situation before; **accuracy is the slower layer** — recognition proves a seat *played*, not that its judgement was *good*, so as threads resolve observably their **confirmed** outcomes (walled in software from the *believed* merge) are scored by a strictly proper rule (Brier). Believability earned by calibration is the destination; auditable recognition is what ships first. Full calibration infrastructure is deliberately deferred — the hardest part to operationalise honestly — but the believed/confirmed wall and confirmed resolutions on fast-resolving threads are cheap and real, so the practice consults the record, never grading itself.
...
16. **Threads and Streams are distinct — outcome-first vs action-first.** A **Thread** is the world view's own belief over a narrative **outcome** (will X happen? — a stance over how a question resolves). A **Stream** is one perspective's leaning over the **candidate actions** it could take on an open *question of what to do* (member-owned, its priors are its log, the result of any move downstream and uncertain). Threads track outcomes; Streams calibrate the turning of thought into action. Don't use "thread" for a member's per-seat action-bearing, or "stream" for the canonical narrative belief, and don't frame a Stream as an outcome prediction.

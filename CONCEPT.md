...
> **Implementation spec — Conviction, the Rehearsal card game.** Conviction is the War Room played as a **turn-based card game** on the shipping substrate — threads/stances, **streams/priors**, **merges**, **locations** ([CLAUDE.md](CLAUDE.md)). It is the **gamified core of the rehearsal engine**: the game people gather at free **workshops** to play, and the activity the **social network compounds around** — every session's judgement and worlds accruing to a client-owned substrate. This doc is implementation-first: the **game loop as a state machine**, and the **type model** that backs it (what we reuse, what's net-new). Vocabulary: [LANGUAGE.md](LANGUAGE.md).

---

## In one paragraph

**Conviction is a game of fates** — each seat shapes the **fate of its character**, and of the world, through the **conviction it holds in its beliefs**. It's the engine's thesis made playable: **Fate** is the force, the world view's **streams / stances** are the beliefs, **conviction** is what moves them — so to have influence is to back what you believe hard enough that the world bends to it. Round by round, perspectives (allies, rivals, the world itself — human or agent) are dealt **cards** — concrete claims on open questions, priced by how *unlikely* they are — and spend scarce conviction to **play** them, *face-up* to signal or *face-down* to hide. The table's commitments fold into a **merge** the engine plays forward into a kept **branch** you then live in. At root it **turns the tedious work of simulating outcomes into a social process**: players choose the actions they'll play, then the world is **simulated one step forward** — forecasting reframed as a table everyone reads and bets on, rather than a solitary calculation. It's **Capture run live** — the same streams, priors, and resolver, a week compressed into a session. No assigned goals; the spine is **agency + consequence**, scored only by how much **Fate** you drove.
...
- **Rounds** *(default — slower, more strategic)* — a **sequential, deterministic phase machine**: one known state at a time, seats moving through the three player phases **READ → WRITE → PLAY** (PLAY in poker turn order, rotating button). You watch the table commit before you do. The canonical loop, specified below.
...
**One window, automated.** Conviction is a **gamified layer over the shipping stream / merge / generate UI** — it doesn't replace those surfaces, it **wraps and automates** them into one continuous, immersive window for GM and players. The **board is the shared surface** everyone watches the game through (§Surfaces); the **Generate Panel is the GM's progression control inside that same window** — a GM-only overlay on the board, not a second screen — through which the GM advances the turn in **one click** (the three generation calls — perspectives, stream/intuition + deal, the arc — plus pricing and income/decay all run automatically). Results render onto the board for the table. The GM can always **override** — expand that control to drop to the underlying stream/merge tools and edit — because they're the GM. Players never see the Generate Panel; they see only the board and their hand.
...
(Perspective Gen) ─▶ READ ─▶ WRITE ─▶ (Stream & Intuition Gen) ─▶ PLAY ─▶ (Arc Gen) ─┘
   generation         ── player phases ──     generation          player      generation
```

**The loop — three player phases between three generation calls.** Every turn alternates an engine **generation call** (the world writes) with a player **action phase** (the table acts), and closes on the GM's **confirm** (or an auto-pass). The general shape is **generation → action → confirm**, and the action decomposes into the three things a player actually does: **READ → WRITE → PLAY**. In the live UI the active tab **auto-advances** with the phase — READ → the **Perspective** tab, WRITE → the **Write** tab, PLAY → the **Board** (+ the hand in the bottom dock).

**Read / Write / Play is all a player carries — the rest is the GM's.** Poker-inspired, the player's whole loop is the three beats **Read → Write → Play**; the fourth, **Generate** (the world simulated one step forward), is the engine's, advanced by the GM. Players never touch the machinery — the **GM handles the details and keeps the room social and fun**. That lightness is what makes Conviction an **onboarding** experience anyone can join in one sitting.

1. **SETUP** *(GM, desktop)* — GM builds the `GameRoom` across two steps. **The table:** **seats players** off a searchable, prominence-sorted roster — any entity with agency (characters, plus load-bearing **locations / artifacts**), with an optional **AI-suggested cast** drawn from the recent story — each assigned a driver (`human` / `agent` / `gm-proxy`; a **human seat must bind a registered `Member`**, and any seat with no member is **filled by an agent**). **The rules:** resolution (`RESOLVE_BIAS` + the optional Showdown spotlight) and **phase clocks** (the WRITE and PLAY windows), with the **conviction economy under Advanced**. *(No separate locations step; the shipped build runs the `rounds` variant in `computer` mode — single screen, GM proxies everyone, start immediately. `showdown` / `remote` are deferred.)*
2. **WAITING** *(remote only — deferred)* — a waiting room: seats join via per-seat link/QR (`status: pending → joined`). GM **starts** when ready; unfilled competitive seats fall back to `agent`.

**— Generation call: PERSPECTIVE GEN** *(`public-narration`, off-clock)* — the engine generates the scene's **perspectives** off the canonical continuation: the **public** account that brings the table up to date, then **each participating seat's private `PerspectiveView`** — its special context (continuity + vantage). Generated together (parallel fan-out), delivered public-first. The raw **canon stays GM-only**; players only ever see the public layer and their own private view. (Round 1 narrates the opening state.)

3. **READ** *(player — `read-write`)* — each seat **reads the perspective** generated for the current scene: the public account + its own private view, in one **Perspective** surface. The brief — where the world stands, and what this seat sees that the others don't. Players may also pull **non-canon views** of past scenes while reading. *(UI auto-switches to the Perspective tab.)*
4. **WRITE** *(player — `read-write`)* — each seat **works the model** on the **Write** surface: **opens new streams it owns** by **posing open questions** of what to do, and **adds priors** to its streams. A prior that moves a thread the seat's way **cheapens** the cards on it (push `p` up → its `−ln p` cost falls) *and* earns conviction — so writing is where **emergent play** comes from: you reshape **your own** streams before you bet them. A seat may also take its **one move** here — a single hop on the location tree, a signal of intent that re-scopes its whisper channel and rides into the merge. Bounded by the **WRITE clock**. *(UI auto-switches to the Write tab.)*

**— Generation call: STREAM & INTUITION GEN** *(off-clock)* — the engine **seeds each seat's candidate streams + their opening intuitions** (engine-sampled questions grounded in the seat's vantage, with first-pass stances) and **prices + deals the cards** from the worked stance. The deal is the hand a seat takes into PLAY.

5. **PLAY** *(player — `play`)* — **poker turn order** (rotating button). On its turn a seat **plays** card(s) on **its own streams** *face-up* (cheaper, telegraphs the call) or *face-down* (`× FACEDOWN_PREMIUM`, concealed — but force-flipped if contested at the merge), **raises**, **passes**, or **folds** — every card costs `≥ COST_MIN > 0`, up to `CARDS_PER_ROUND` commits total, so a seat can't flood the merge with cheap near-certain cards. Agents auto-resolve their turn from their feed. A seat whose **PLAY clock expires with nothing committed** **cedes** — leaving the question to the LLM, which the rest of the table's commitments already sway. *(UI auto-switches to the Board; the hand sits in the bottom dock.)*

**— Generation call: ARC GEN** *(`resolve` — the GM's confirm)* — the GM closes the turn through the **Generate Panel** (or it auto-passes under auto-resolve). First the **reveal window**: with `FORCED_REVEAL` on every play flips open; with it off, any **contested** thread (two seats' streams on conflicting outcomes) **force-flips** all bearing plays before settling per **`RESOLVE_BIAS`** (a challenged claim is always called), while uncontested plays may stay dark — a **voluntary** reveal of an outcome that resolves **executive** earns a reputation mark + optional `REVEAL_REFUND`. Then the played cards **pre-fill the merge's resolutions** (executive = drives the continuation; recorded = rides along); the GM may override or click through. The **Merge → the engine generates the continuation arc** → a kept branch entry. Folded into the same confirm, in order: an optional **SHOWDOWN** spotlight (`SHOWDOWN_PHASE`, shows how each contest settled), **SETTLE** (decay each banked balance `× α ≈ 0.83`, then grant income), and **SCORING** (the round's **Impact** decomposed across the seats that moved each thread — §The scoring math — banked into `Seat.fateImpact`, shown as the round readout + **Ranking**). The next turn's **Perspective Gen** opens the loop again.

*(`RoundState.phase` ids — `public-narration` · `read-write` · `play` · `resolve`, plus the folded `showdown` / `settle` / `scoring` and the deferred `private-narration` / `live` — back the machine; the loop above is how it reads to the table. `WAITING` (remote) and the `Showdown` variant are specified but deferred.)*

### Showdown (the real-time loop) — *deferred, not in this build*

Same skeleton, but **READ, WRITE and PLAY collapse into one concurrent LIVE phase** — there is no turn order, no rotating button, no per-seat clock. All seats act at once against a single shared `TIMER_LIVE`. *(Specified here for completeness; the shipped build runs Rounds only, in the Read → Write → Play form above.)*
...
| `TIMER_PUBLIC` | 30s | READ window — reading the public account; short, it's a read |
| `TIMER_PRIVATE` | 20s | READ window — your private brief; short |
| `TIMER_READ` | 120s | *(Rounds)* the **WRITE** window — open streams + add priors; the strategic heart, given room |
| `TIMER_TURN` | 30s | *(Rounds)* per-seat **PLAY** clock (timeout → no action / cede) |
| `TIMER_LIVE` | 120s | *(Showdown)* shared real-time window — all seats Read/Write/Play at once; closes on timer or all-locked-in. Replaces TIMER_READ + TIMER_TURN |
| `TIMER_SHOWDOWN` | 25s | *(optional, within Arc Gen)* contested-thread spotlight before SETTLE |
| `TIMER_SCORING` | 20s | round readout + Ranking — short |
| `READ_MAX_REQUESTS` | 3 | card-requests allowed per WRITE / live phase (bounds the branching loop) |
...
*(Pacing split: the generation calls (Perspective / Stream & Intuition / Arc) are **off-clock**; the **player** phases carry the clocks — READ short (it's a read), **WRITE** the strategic heart (`TIMER_READ`), **PLAY** the per-seat commit clock (`TIMER_TURN`). All are GM dials under Advanced settings — and the GM can grant `+time` on the live clock mid-phase.)*
...
| `CONVICTION_INCOME` | 25 | granted each SETTLE — **≈ one typical multi-action call** (a 4-action stream ≈ 28); ~one real move a round, anything bigger needs saving |
| `CONVICTION_DECAY` | `5/6 ≈ 0.83` | tax on the **banked** balance each SETTLE (before income). Fixed point `INCOME/(1−DECAY) = 150` is the **hoard ceiling** — a fully-banked saver converges here, never a runaway war chest. A GM **No-cap accumulation** toggle removes it: banked conviction stops decaying and grows unbounded (`settle = balance + income`). *(Decoupled from the stance's `STANCE_VOLUME_DECAY = 0.9`.)* |
| `COST_MAX` | 200 | card-cost **ceiling** — the dearest a single play can price, however rare (GM dial). A **No-cap** toggle removes it so the rarity curve runs **unclamped** (`cost = round(RARITY_SCALE·−ln p)`). |
...
*(Economy is scaled to the **`COST_MIN`–`COST_MAX`** range, `COST_MAX = 200` by default and **removable** (No-cap → the rarity curve runs unclamped). The hoard ceiling `INCOME/(1−DECAY) = 150` is the soft cap a perpetual saver converges to — with the default cost ceiling the very rarest concealed call can outrun it, and the **No-cap accumulation** toggle drops the ceiling entirely. Income ≈ one typical multi-action call, so each round buys ~one real move and a **50 opening affords ≥1.5 typical cards**; a face-up **long-shot (~46) takes ~2 rounds** of saving — the signature long con stays a costly, occasional event. Ratio: **start = 2× income**.)*
...
**Rarity → cost** — the price of committing a card, on an intuitive **`COST_MIN`–`COST_MAX`** scale:

```
cost = clamp(COST_MIN, COST_MAX, round(RARITY_SCALE × −ln p))   // p = outcome's live streamProbs
RARITY_SCALE = 20   ·   COST_MIN = 1   ·   COST_MAX = 200 (GM dial; No-cap removes it)
```

So near-certain (`p≥0.97`) floors at **`COST_MIN`** (cheap, but never free), a modal call (`p≈0.7`) ≈ **7**, a coin-flip (`p=0.5`) ≈ **14**, a typical 3–5-action call (`p≈0.2–0.33`) ≈ **22–32**, a long-shot (`p=0.1`) ≈ **46**, a 1-in-20 (`p=0.05`) ≈ **60**, anything rarer climbs toward **`COST_MAX` (200 by default)** — or runs **unclamped** when the GM removes the ceiling. Improbability *is* the price — tuned so a 50-conviction opening buys ≥1.5 typical cards.
...
- **`remote` mode — distributed.** Players join their own **mobile controller** over a **Cloudflare quick tunnel** (`cloudflared`) — a session-scoped public **HTTPS URL** with a real cert, so a per-seat **QR code just works** on any phone (no app install, no account); a **waiting room** holds the round until seats are `joined`. WebSockets ride the same tunnel, so the live controller sockets are fine. The controller is the lighter **player interface**: **move, chat, read** (perspective feed + hand). *(Least-settled half — see Open questions.)*
...
- **Sovereign host, two doors.** The GM runs the **sovereign host** (Electron app + daemon; state in IndexedDB; **no central server**). Non-GMs reach it two ways, and the load-bearing rule is: **the daemon trusts nothing from either door until it's bound to a `Member` and gated — tunnel requests by session token, Signal messages by sender UUID. Same rule, two doors.**
  - **Live transport — Cloudflare quick tunnel.** The controllers ride a **`cloudflared` quick tunnel** — *public* exposure managed by Cloudflare, zero-account: `cloudflared tunnel --url localhost:PORT` returns a random `https://….trycloudflare.com` with a real cert. TLS terminates at Cloudflare's edge, so **application-layer auth is the entire perimeter**: two-stage pairing (**token QR + GM-screen PIN**), and the daemon **rejects all unauthenticated traffic except the rate-limited pairing endpoint** (the public hostname draws background scanner noise). **Implementation:** bundle the `cloudflared` binary (electron-builder `extraResources`) or fetch on first run; on SETUP **spawn it as a child process**, parse the assigned `trycloudflare.com` URL from stderr into the seat QRs, and **kill it when the room closes** — so exposure is **temporal, not permanent** (the random URL dies with the session). Fully-private / same-room tables can run **LAN-only** (no tunnel). Design around its limits: HTTPS only, relayed best-effort bandwidth (no SLA on quick tunnels) — fine for board + chat; route audio / heavy media as client-side fetches or keep it on LAN tables.
  - **Async capture — Signal (the one capture channel).** Capture (priors between sessions, **not** in-game chat) rides **Signal**, end-to-end encrypted (via `signal-cli`: a **dedicated room number**; members **DM** their priors — *not a group*, which would leak every seat's beliefs; bound **sender-UUID → Member**; **quarantined one-way ingress** forwarding only `(sender, text, timestamp)` to the daemon over localhost, wiping its local store, replying **only a bare admissibility ack** — "admitted, T3 moved" / "denied, rephrase," never narration or game state, so a compromised channel leaks one member's own words and nothing else). That ack is the **tight in-flow loop**.
- **In-game chat stays sovereign.** Table talk (global / location scope) is a **game mechanic, not a messaging feature** — location-gated, perspective-gated, agents participate, and it feeds the scorer's read of soft factors. It lives **in the controller**, never on Signal.
- **GM = sovereign operator** (full state, the master console); **players = guest seats** with no authority over truth and no view of others' hidden state.
- **Surfaces (the build) — minimalist, board-centric.** **The Board** is the home surface (poker-table view), and a thin **tab strip** carries the three player phases the turn auto-advances through: **Perspective** (READ — public + private view of the current scene), **Write** (WRITE — streams + priors), and the **Board** itself (PLAY — the hand in the bottom dock). An always-visible **phase + timer bar** sits above the dock on every tab (the GM can grant `+time`). **Chat** is a full-height messenger tab — global persists across the game, location whispers are **round-ephemeral**, and `@`-mentions raise a notification badge. **Navigation** (move, pose-question / request-more, **set / reassign goal**, request-quest, settings) is via **popups** layered over the board. The **Impact tally** (Fate moved per seat) rides on the board as the live leaderboard; the end-game **debrief** is a per-seat readout. The **GM's progression control is the existing Generate Panel, surfaced as a GM-only overlay** (see *One window, automated*) — one screen for everyone, with the GM's advance/override controls layered on top. Rendered global for the GM and perspective-gated for players. Component map: [MERMAID.md](MERMAID.md) §8.
- **Built for the laptop / desktop, not the phone.** Conviction is a **read-and-write game**: orienting on a play means *reading* the perspective that opened the scene and the table state, and making one means *writing* — opening streams and authoring the legible priors that move the odds. Reading and writing are the cornerstones of how you understand the board and act on it, and both want screen real estate and a keyboard, so the full experience targets **laptop / desktop**. Mobile is **not** a play surface for it; the only phone-shaped client is the deliberately **lightweight `remote` controller** (deferred) — move / chat / read, no authoring — for a player who isn't in front of a computer.
...
- **Live session security** — *decided:* **Cloudflare quick tunnel** (`cloudflared`, spawned per session, killed on close) with app-layer two-stage pairing (**token QR + GM PIN**), a rate-limited pairing endpoint, and exposure that dies with the session; LAN-only fallback for fully-private tables. Open: pairing-token lifetime, re-issue / revoke when a seat changes hands, and bundling vs first-run-download of the binary.

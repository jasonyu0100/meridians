...
/**
 * CaptureView — daily-ingest workspace rendered as a canvas mode.
 *
 ...
 * StagePalette "go to search") land in the right sub-mode
 * automatically.
 *
...
export function CaptureView() {
  ...
  const entries = useMemo<Prior[]>(() => {
    if (!narrative) return [];
    return Object.values(narrative.priors ?? {}).sort(
  ...
  async function generateEntry(direction: string, sourceUrl?: string) {
    try {
      const { title, text } = await generatePrior(narrative, direction, sourceUrl);
      const id = `entry-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`;
      dispatch({
        type: 'CREATE_PRIOR',
  ...
  function updateActive(updates: { title?: string; text?: string }) {
    ...
    dispatch({ type: 'UPDATE_PRIOR', entryId: activeEntry.id, ...updates });
  }

  function deleteEntry(entry: Prior) {
    if (isLocked(entry)) return;
    dispatch({ type: 'DELETE_PRIOR', entryId: entry.id });
    setSelectedIds((prev) => {
...
function EntryGroup({
  ...
  onDelete: ((entry: Prior) => void) | null;

...
export function SlidesPlayer({
  ...
  resolvedKeys: string[];
  /** Narrative-start → end-of-window keys, so the belief system accumulates
   *  prior arcs even for a region deck. Defaults to `resolvedKeys` (full deck). */
  cumulativeKeys?: string[];
  /** When set, the deck is scoped to a named region (e.g. an arc range); shown
   *  in the header and on the title slide. Unset = full-narrative summative deck. */
  scopeLabel?: string;
  onClose: () => void;
}) {
  const slidesData = useMemo(
    () => computeSlidesData(narrative, resolvedKeys, cumulativeKeys ?? resolvedKeys),
    [narrative, resolvedKeys, cumulativeKeys],
  );
    ...
    <div ref={containerRef} className="fixed inset-0 z-modal bg-bg-base flex flex-col outline-none" tabIndex={0}>
      ...
      <div className="flex-1 overflow-hidden relative">
        ...
        <div className={`h-full overflow-y-auto transition-opacity duration-200 ${transitioning ? 'opacity-0' : 'opacity-100'}`}>
          {renderSlide(currentSlide, slidesData, onClose, scopeLabel)}
        </div>
...
function renderSlide(spec: SlideSpec, data: SlidesData, onClose: () => void, scopeLabel?: string): React.ReactNode {
  switch (spec.type) {
    case 'title':
      return <TitleSlide data={data} scopeLabel={scopeLabel} />;
    case 'paradigm-lens':

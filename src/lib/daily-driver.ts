...
export type SynthesiseDriverCompactInput = {
 ...
 *  MARK_DRIVER_ENTRIES_USED with the new file id once the file is
 *  stored. This function is intentionally pure — no IDB writes, no
 *  store dispatches, no awareness of the active narrative. */
export async function synthesiseDriverCompact(
  input: SynthesiseDriverCompactInput,
): Promise<string> {
  const { entries, compactTitle, onToken } = input;
  if (entries.length === 0) {
    throw new Error('synthesiseDriverCompact: no entries selected');
  }

  const ordered = [...entries].sort((a, b) => a.capturedAt - b.capturedAt);
  const prompt = buildDriverSynthesisPrompt({

...
const DRIVER_ENTRY_SYSTEM = `You produce a single Driver ENTRY — a short, structured fragment of thought captured into an operator's daily-ingest workspace.
...
export async function generateDriverEntry(
  ...
  if (!trimmedDirection && !trimmedUrl) {
    throw new Error('generateDriverEntry: direction or sourceUrl required');
  }
  ...
  try {
    ...
    logError('generateDriverEntry call failed', err, {
      source: 'ingest',
      operation: 'generate-driver-entry',
    });
    throw err;
  ...
  const parsed = parseJson(raw, 'generateDriverEntry') as { title?: unknown; text?: unknown };
  const title = typeof parsed.title === 'string' ? parsed.title.trim() : '';
  const text = typeof parsed.text === 'string' ? parsed.text.trim() : '';
  if (!text) {
    throw new Error('generateDriverEntry: model returned no text');
  }
  return { title, text };
}

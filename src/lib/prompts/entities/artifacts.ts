...
export const PROMPT_ARTIFACTS = `<artifacts hint="Concrete things an entity can USE, possess, cite, transfer, or invoke under the rules. Match the register the work establishes — instruments and documents that belong in this specific world.">
  ...
  <example type="good" kind="rule-bearing-instruments">a binding treaty that constrains state behaviour, a sect's succession charter that gates ascensions, a cultivation pill whose effect is fixed by the world's rules, a wargame turn-report whose tally binds the next decision, a calibrated measurement device that certifies a finding — concrete instruments whose use triggers stated mechanisms.</example>
  <example type="good" kind="simulation-initial-conditions">a 1962 NSC briefing folder, a Politburo decision-rule playbook, a Joseon admiralty's monsoon-window dispatch, a sect's succession charter, a regional grain ledger — concrete in-world documents the modelled events run from. (NOT "the calibration table the modellers used" — that's meta infrastructure.)</example>

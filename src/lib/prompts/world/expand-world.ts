...
${continuityBasis ? `  <priority rank="0">CONTINUITY-BASIS — committed reality the expansion extends from. The merged resolutions are ground truth: don't introduce entities or threads that contradict a committed outcome, and DO add the entities / threads that each committed outcome concretely calls for (its actual content must shape what you add, not just its topic). The priors under them are NOISY perspective-held thinking — de-noise them via the block's &lt;synthesis&gt; into the net directional pressure.</priority>\n` : ''}  <priority rank="1">DIRECTIVE / SOURCE-MATERIAL — explicit creative brief; the expansion must serve these directly. Source-material (when present) is verbatim authority over names, roles, and specifics.</priority>

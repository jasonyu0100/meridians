...
/** World-shape each paradigm maps to. Five shapes:
...
const PARADIGM_SHAPE: Record<NarrativeParadigm, { shape: ParadigmShape; directive: string }> = {
  'fiction':      { shape: 'populated-narrative',      directive: 'REALITY POSTURE: invented. The world is wholly authored — no entity, event, or rule needs to be sourceable to anything external. Populated scene-narrative; Fate (thread resolution) + World (character transformation) carry the weight. Apply the populated minimums; match the register and setting the premise implies. The distinction from non-fiction is exactly this reality posture; same form, different sourcing discipline.' },
  'non-fiction':  { shape: 'populated-narrative',      directive: 'REALITY POSTURE: observed. The world IS the documented record — every named person, place, event, date, and detail must be anchored to actual fact. Where the premise names real figures, honour historical accuracy: the engine narrates ABOUT what happened, it does not invent what happened. Populated scene-narrative; same form as fiction. The distinction from fiction is exactly this reality posture: fiction invents, non-fiction documents. Where the premise leaves gaps in the record, name the gap rather than fabricate.' },
  'simulation':   { shape: 'rule-governed-narrative',  directive: 'REALITY POSTURE: hybrid — real rules acting on real or invented agents. The rules are sourceable / explicitly stated (Politburo decision rules, monsoon logistics, sect succession charter, electoral allocation, monetary-policy mechanism); the agents may be real (Khrushchev, Yi) or invented under the rules. Forward-time event modelling: scenes narrate what the RULES force as conditions evolve. System rules are LOAD-BEARING — they drive what happens, not character agency. Threads close on rule-driven consequences, not authorial choice. (If you want a team interpreting existing evidence, pick PANEL; if you want a two-sided contest, pick DEBATE.)' },
  'essay':        { shape: 'singular-thinker',         directive: 'REALITY POSTURE: observed evidence + real author. The author is named (real or, rarely, a clearly-labeled invented persona); the evidence engaged with is sourceable. One named anchor (the author), 1-3 transient cited interlocutors (theorists, methods, primary-source authors the work engages or rebuts). The system-graph IS the argument substrate; internal friction substitutes for multi-voice disagreement. Threads track the argument-questions the author is pursuing.' },
  'panel':        { shape: 'multi-thinker',            directive: 'REALITY POSTURE: observed evidence + named cast (AI agents OR human experts). Evidence is existing — LLM knowledge + user\'s source material; the cast is the imagined / assembled team that reasons over it. A named cast of 2+ thinkers — AI agents (Atlas, Cipher, Nexus) OR human experts (Dr. Wen, Prof. Vasquez) — pursuing a shared question. Cooperative-with-disagreement: the work IS the contest of minds becoming a synthesis. Includes a devil\'s-advocate role + ≥1 adversarial pair. Scenes are COGNITIVE events over existing evidence — NOT fabricated forward-time events. (If you want one author, pick ESSAY; if you want zero-sum adversarial structure, pick DEBATE; if you want rules-acting-on-modelled-world, pick SIMULATION.)' },
  'atlas':        { shape: 'reference-typology',       directive: 'REALITY POSTURE: either — a real-world atlas (a flora, an encyclopedia of jurisdictions, a doctrine corpus) OR an invented-world codex (the sects of an invented secondary world). Each entry must be internally consistent within whichever posture the premise chooses. No fate threads, no character transformation; the SYSTEM force is everything — the typological structure IS the work. "Entries" replace scenes. The curator orchestrates; specimens / categories / entities are the populated cast. Threads (if any) track classification questions ("does this specimen belong in family X?"), not dramatic resolution.' },
  'debate':       { shape: 'adversarial-contest',      directive: 'REALITY POSTURE: either — a documented contest (a real trial, a historical election, a real championship) OR a hypothetical one (a scripted moot, an invented negotiation). Rules of engagement are typically real / sourceable even when the contestants are invented. Two or more named parties locked in a zero-sum contest under explicit rules. Fate (who wins each axis) + System (rules of engagement) carry the weight. Each scene is a MOVE in the contest; threads track each axis of contestation; parties are anchors whose capacities and stakes are tracked across the contest. (If you want cooperative-with-disagreement, pick PANEL; if rules drive events without two-sided agency, pick SIMULATION.)' },
};
...
Return JSON with this exact structure:
{
  ...
  "genre": "Primary genre WITHIN the chosen paradigm — e.g. for fiction: fantasy, sci-fi, thriller, romance, horror, mystery, literary; for non-fiction: biography, history, memoir, reportage; for simulation: counterfactual, wargame, policy modelling, cultivation; for essay: empirical (research-finding), theoretical, personal, critical, polemical (manifesto); for panel: macro-strategy, investigation, multi-agent reasoning, advisory; for atlas: field guide, codex, doctrine, encyclopedia; for debate: trial, election, championship, M&A, structured debate.",
  "subgenre": "Specific sub-form within the genre — e.g. progression fantasy, cozy mystery, autobiographical memoir, Mughal-succession counterfactual, monetary-policy wargame, geopolitical macro strategy, applied-econometrics essay, criminal trial, hostile acquisition. Pick the most identifying form.",

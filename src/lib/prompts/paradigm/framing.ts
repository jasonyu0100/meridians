...
export const COMPASS_FRAMING_BY_PARADIGM: Record<NarrativeParadigm, string> = {
  ...
  'game':        'COMPASS MODE: strategic recommendation under the rules. The factor model surfaces game drivers (actors\' positions, resource stocks, information asymmetries, alliance vectors, rule-trigger proximities, stake-pressure); the cohort recommends next moves available to the actors in the current state, scored by strategic leverage under the game\'s own rules. PriorLogits express how the rules + current state pull toward each move; this is not an external probability forecast over real-world outcomes.',
};

...
export const PROSE_SHAPE_DIRECTIVE_BY_PARADIGM: Record<NarrativeParadigm, string> = {
  ...
  'scenario': `<paradigm-shape paradigm="scenario" critical="true" hint="A MOVE in a modelled strategic moment. The operative dynamics bear; the stakes are live.">
  This scene is a MOVE — one actor's decision or action in a modelled strategic moment, read from its own vantage. The modelled state is canonical; the operative dynamics shape what follows; what each actor knows is gated by its position and intelligence.
  Render: which actor moved (attribution), what they committed to given their goal, hand, and read of the moment, which dynamic or constraint bore on it, what position / leverage / information / stakes shifted, what the other actors can see (full or partial, per their vantage).
  DO NOT narrate from an omniscient emotional throughline — a scenario has no single protagonist; each actor is its own perspective with its own stakes, information, and constraints. DO NOT smooth a move past the constraints that actually governed the moment; consequences follow the dynamics, not authorial preference. DO NOT collapse the cast into one monolithic voice; actors are distinct.
  The dynamics are load-bearing and surface diegetically — when a pressure bites (a deadline lapses, a supply line stresses, a commitment triggers), deliver it as the moment would. The live stakes (the open strategic questions) tip only as the dynamics force them, not when the narrative would prefer.
</paradigm-shape>`,
};
...
export const WORLD_ARCHITECT_BY_PARADIGM: Record<NarrativeParadigm, string> = {
  ...
  'scenario':    'You are a world-view architect modelling a STRATEGIC MOMENT — a key event reconstructed for play, where multiple actors pursue their aims from their own vantage under the moment\'s operative dynamics. The system-graph captures those dynamics (the rules, pressures, constraints, and couplings that governed the moment); world entities are the ACTORS (decision-makers / factions) and the RESOURCES, POSITIONS, and ARTIFACTS they command; threads are the LIVE STAKES the moment hangs on (what each actor is deciding, what could tip). Model the actors\' goals, capabilities, constraints, and information asymmetries up front; they bind every downstream pass. Ground in the real moment; name gaps rather than invent.',
};
...
export const WORLD_SHAPE_LABEL_BY_PARADIGM: Record<NarrativeParadigm, string> = {
  ...
  'scenario':    'strategic-scenario — a real moment modelled for play; multiple actors pursue their aims from their own vantage under the moment\'s operative dynamics; system-graph captures those dynamics, world tracks actors + capabilities + positions + artifacts, threads are the live strategic questions the moment hangs on (REALITY POSTURE: a real strategic moment, historical or contemporary, reconstructed for rehearsal)',
};
...
export const SCENE_GENERATOR_FRAMING_BY_PARADIGM: Record<NarrativeParadigm, string> = {
  ...
  'scenario':    'You generate MOVES in a modelled strategic moment — each scene names the acting actor, the move they committed to from their own vantage, the operative dynamic that bore on it, the effect on position / leverage / information / stakes, and what other actors can see. The dynamics govern; consequences follow them, not authorial preference; information asymmetries are respected. NOT a fiction scene with a single protagonist; each actor reasons from its own information.',
};
...
export const PLAN_SHAPE_BY_PARADIGM: Record<NarrativeParadigm, string> = {
  ...
  'scenario':    'You are planning a MOVE — beats sequence the acting actor\'s deliberation (from its own information), the chosen move (given its goal and hand), the dynamic that bears (which pressure or constraint shapes it), the state change (position / leverage / stakes shift), and what is disclosed to other actors. Each beat anchors to the acting actor + a move + a dynamics-shaped outcome.',
};
...
export const EDIT_SHAPE_BY_PARADIGM: Record<NarrativeParadigm, string> = {
  ...
  'scenario':    'scenario move (acting actor + a move from its own vantage + a dynamics-shaped outcome + state change + what other actors can see)',
};
...
export const ANALYST_DISCIPLINE_BY_PARADIGM: Record<NarrativeParadigm, string> = {
  ...
  'scenario':
    'Read for strategic dynamics: which actor moved, what they committed to from their vantage, which dynamic or constraint bore and how the state changed, which stakes shifted, what each actor knew vs. didn\'t going in. Threads are the live STRATEGIC QUESTIONS the moment hangs on — they tip as the dynamics force, not when the analyst would prefer. Each actor is its own perspective with its own goals, information, and constraints; do not impose a single-protagonist throughline on a multi-actor moment.',
};
...
Pool variables name TRAJECTORY FORCES — long-running trends, regime-shift pressures, figures whose state is shifting, institutional rhythms, chronicled patterns. Do not invent events the chronicler couldn't have witnessed at the declared velocity.`,
  ...
  'scenario': `COMPASS LENS — scenario.
"Continuations" are next moves available to the actors in the modelled moment: actions open from the current state, responses to others' last moves, alliance shifts, resource gambits, plays on the live stakes, the pressure the dynamics are building toward.
Forward-motion shape: move-and-counter-move — each actor's move reshapes the others' options; the cohort surfaces the strategic postures each actor might adopt and the consequences the dynamics then force.
Native attractors: the moment's own attractors (the ways the stakes can resolve, the equilibria, the terminal states), each actor's stated aims, the live questions the moment is deciding.
Natural cadence: paced by the dynamics — moves resolve as the moment allows, state updates as pressures trigger. Subgenre adjusts pace (a crisis moves faster than a campaign season faster than a negotiation faster than a market week).
Tail vocabulary: a sharp tip — a stake resolving, a new actor entering or exiting, a constraint activating that voids prior leverage, an alliance collapsing, a deadline lapsing, hidden information becoming common knowledge.
Pool variables name STRATEGIC FORCES in the moment — actors' positions, capabilities and resources, information asymmetries, alliance vectors, the operative dynamics and their pressure points, the pressure on the live stakes. Score strategic state under the moment's own dynamics; do not invent forces the model cannot express.`,
};
...
export const SURVEY_PROBE_HINT_BY_PARADIGM: Record<NarrativeParadigm, string> = {
  ...
  'scenario':    'Probes that pay: each actor\'s aim (what they\'re actually after), their read of their own options vs. opponents\', the move they most fear, the resource they\'d trade everything for, the leverage they\'d exploit if they could, what they don\'t know that others do. Forced-rank questions surface priorities; estimate questions surface information asymmetries between actors.',
};
...
export const INTERVIEW_PROBE_HINT_BY_PARADIGM: Record<NarrativeParadigm, string> = {
  ...
  'scenario':    'The subject is one actor in the moment — interview surfaces their strategy, their read of the dynamics and the current state, the leverage they hold, the moves they fear, the alliances they\'re calibrating, what they know vs. don\'t. Answers are positioned in the modelled state; the actor responds from its own vantage, never from a global view.',
};
...
export const DIRECTION_SHAPE_BY_PARADIGM: Record<NarrativeParadigm, string> = {
  ...
  'scenario':    'The next "arc" is the next BEAT of the moment — which actor moves next, which stakes are pressing, which dynamics are about to bite, which resource / position / alliance shift opens new moves. Vector, not script — resolution belongs to the dynamics, not the analyst. Do not script which actor "should" win.',
};
...
export const EXPANSION_SHAPE_BY_PARADIGM: Record<NarrativeParadigm, string> = {
  ...
  'scenario':    'Extend the modelled moment — add ACTORS (decision-makers / factions), RESOURCES (artifacts the actors command, with capacity where it matters), POSITIONS (locations / terrain in play), and dynamics as system nodes (new pressures, constraints, couplings, ways a stake can resolve). New actors enter with declared aims, capabilities, constraints, and information; new dynamics say which moves they enable, foreclose, or shape. Do not add bystanders who don\'t bear on the moment; everyone modelled has a stake.',
};
...
export const PROSE_REVIEW_CRITERIA_BY_PARADIGM: Record<NarrativeParadigm, string> = {
  ...
  'scenario':    'PROSE CRITERIA — scenario. Dynamics fidelity — every move consistent with the operative dynamics in the current state; constraints not smoothed away. Actor attribution — which actor moved, under which constraint, with which information. State-change traceability — position / leverage / information / stakes shift as the dynamics allow. Multi-actor distinctness — actors keep distinct aims, information, and voices; no monolithic cast register. Vantage discipline — what each actor knows or guesses stays consistent with its position and intelligence. Repetition. Profile compliance.',
};
...
export const BRANCH_REVIEW_CRITERIA_BY_PARADIGM: Record<NarrativeParadigm, string> = {
  ...
  'scenario':    'BRANCH CRITERIA — scenario. Dynamics-driven trajectory — do moves follow as the operative dynamics + actors\' options force? Authorial rescue is a verdict-edit flag. Pacing — move cadence + phase shifts. Repetition (same actor making the same kind of move without the state advancing, or the same pressure firing without the stakes moving). Actor development — each actor\'s position / resources / information evolving distinctly. Threads — the live strategic questions, tipping only as the dynamics force. Thematic question — what the moment under stress reveals about its actors or dynamics.',
};
...
export const PLAN_REVIEW_CRITERIA_BY_PARADIGM: Record<NarrativeParadigm, string> = {
  ...
  'scenario':    'PLAN SHAPE — scenario. Beats sequence the acting actor\'s deliberation (from its own information), the chosen move (given its aim and hand), the dynamic that bears, the state change, and what is disclosed to other actors. Beat-to-delta alignment is CRITICAL: a system delta declaring a dynamic bears must have a beat where it actually bites; a thread (stake) delta declaring a stake tips must have a beat where a move shifts it under the dynamics.',
};

/**
 * Phase application prompts — string composition for injecting the
...
import type { Phase } from "@/types/narrative";
 ...
 */
export function buildModeDataBlock(graph: Phase): string {
 ...
 */
export function buildModeApplicationBlock(scope: PhaseScope): string {
...
const SCOPE_USAGE_BLOCK: Record<PhaseScope, string> = {
 ...
 */
export function buildModeSection(graph: Phase, scope: PhaseScope): string {
 ...
 */
export function buildPriorModeSection(prior: Phase): string {
 ...
 */
export function modePriorityEntry(rank: number, scope: PhaseScope): string {
  const { framing, guidance } = SCOPE_PRIORITY_BLURB[scope];
  return `<priority rank="${rank}">PHASE GRAPH (PRG) — ${framing} ${guidance}</priority>`;
}

const SCOPE_PRIORITY_BLURB: Record<PhaseScope, { framing: string; guidance: string }> = {
  expand: {

...
<example>
{
  ...
  "nodes": [
    ...
    // REASONING NODES — the backbone; carry the FULL inference-node-shape (detail + considered + breaks + opens).
    {"id": "reason-resolution-needs-charter", "index": 9, "type": "reasoning", "label": "Resolution requires securing the charter first", "detail": "The Summit confrontation can only land its peak if the anchor arrives with the charter already in hand — without it the counterpart's leverage holds and the confrontation collapses into another deferral. The charter is the only asset that flips the power balance at the table. Hands off to the valley arc where the charter is actually secured.", "considered": "Could have routed via direct concession (rejected: concession costs the principal their standing-quorum, voiding the Summit's framework). Could have routed via faction defection (rejected: defection requires earlier seeding the plan can't afford this arcSlot).", "breaks": "Breaks if the charter is contested-but-not-securable within the arc-2 window — if the counterpart faction has already filed for arbitration, the charter route closes and the peak needs a different lever.", "opens": "Once the charter is in hand, opens a longer-term arc on the counterpart's response (whether they accept the framework or build a rival one); opens a new market on which faction controls the charter's ratification in subsequent plans.", "arcSlot": 2},
...
<node-types hint="All nodes grounded in SPECIFIC context from inputs above.">
  ...
  <reasoning-nodes>
    <type name="reasoning">Logical step in backward induction. Has arcSlot. Label = inference (3-8 words). Carries the FULL INFERENCE-NODE SHAPE: \`detail\` + \`considered\` + \`breaks\` + \`opens\` (see <inference-node-shape> below). The four fields together expose the multi-arc reasoning machinery so the user can walk the plan and re-evaluate at each step — without the three handles, the plan collapses to a chain-of-thought sketch.</type>
  </reasoning-nodes>

  <inference-node-shape hint="Required on inference-tier nodes (reasoning, pattern, warning, chaos). Spine (peak/valley/moment) and priors do NOT use this shape.">
    <field name="detail" required="true">1–3 sentences. The causal logic of THIS step given predecessors visible to its arcSlot. NOT graph-position metadata.</field>
    <field name="considered" required="true" critical="true">1–3 sentences naming the competing multi-arc routings considered and discarded, with why. Plan-level reasoning has the highest value-add from this — it's where the user sees multi-arc strategy choice. If no rival routing applies, say so explicitly here — never omit silently.</field>
    <field name="breaks" required="strongly-encouraged">1–2 sentences. The load-bearing assumption whose failure voids the step (missing entity, closed window, mispriced rival reaction).</field>
    <field name="opens" required="strongly-encouraged">1–2 sentences. Affordances this grants LATER arcs — markets it opens beyond this slot, entities it activates, threads it spawns.</field>
    <discipline>Every inference-tier node MUST carry \`considered\` (silent omission is logged).</discipline>
...
<requirements>
  ...
  <requirement index="11b" name="inference-shape-on-inference-tier" critical="true">Every INFERENCE-TIER node (reasoning, pattern, warning, chaos) carries the full <inference-node-shape>: \`detail\` + \`considered\` + \`breaks\` + \`opens\`. Plan-level reasoning has the highest value-add from these handles because the user is comparing multi-arc strategies; without \`considered\` the plan looks chosen but not selected, without \`breaks\` it hides assumptions, without \`opens\` it doesn't expose how this arc compounds into later ones. Spine nodes (peak/valley/moment) and priors do NOT carry the shape — only inference-tier nodes.</requirement>

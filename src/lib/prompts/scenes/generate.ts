...
/** Multipurpose fallback when paradigm is unset — preserves the legacy
 *  prompt verbatim so paradigm-less narratives keep working unchanged. */
const SCENE_GENERATOR_FALLBACK =
  'You are a scene generator producing one arc of structurally rich scenes. The work may be a fiction or non-fiction narrative, a rule-driven simulation, an essay, a panel session, a typology, an adversarial contest, or a chronological record — read the paradigm-shape block in the user prompt to learn which, and produce scenes whose shape MATCHES the paradigm (a typology scene is a typology entry, not a sequence of events; a debate scene is a contest move, not a character\'s emotional arc; a record scene is a dated entry, not a forward-time narrative beat). ' + SCENE_GENERATOR_SHARED_DISCIPLINE;

/** Build the scene-generation SYSTEM prompt. Case-based on the
 *  operator-declared paradigm. When paradigm is unset, falls back to the
 *  legacy multipurpose preamble. */
export function buildGenerateScenesSystem(work?: WorkIdentity): string {
  if (!work?.paradigm) return SCENE_GENERATOR_FALLBACK;
  const identity = work.title
    ? composeWorkIdentity({
        title: work.title,
    ...
    : '';
  const framing = SCENE_GENERATOR_FRAMING_BY_PARADIGM[work.paradigm];
  return [
    identity,
    framing,
    SCENE_GENERATOR_SHARED_DISCIPLINE,
...
const PARADIGM_SCENE_SCENARIO = `<paradigm-scene-discipline paradigm="scenario" critical="true" hint="Scenario. Each scene is a MOVE or development in the modelled strategic moment — an actor decides or acts from its own vantage, the operative dynamics bear on it, the live stakes shift, and the other actors read it through what they can see.">
  <rule name="move-is-the-unit" critical="true">Each scene is a strategic MOVE or development. Surface (a) the ACTING actor (whose decision or action this is), (b) the MOVE (what they commit to, given their goal, their hand, and their read of the moment), (c) the DYNAMICS that bear (which pressure, constraint, deadline, or rule shapes what follows), (d) the EFFECT (how position, leverage, and the live stakes shift).</rule>
  <rule name="dynamics-bear">The system-graph is the operative dynamics — escalation ladders, commitments, supply and time limits, market structure. Scenes honour them: a move runs into the constraints that actually governed the moment, and consequences follow the dynamics, not authorial preference.</rule>
  <rule name="threads-are-live-stakes">Threads are the live strategic questions the moment hangs on. They shift as moves land and resolve when the moment tips. Payoff / twist logTypes are reserved for moves that genuinely tip a stake.</rule>
  <rule name="actors-have-distinct-vantage">Each actor reasons from its own information — what it knows, guesses, and misreads. Asymmetries are deliberate: what one actor sees of another is gated by position and intelligence, never a global view.</rule>
  <rule name="no-single-protagonist">A scenario has no single protagonist. POV moves between actors; the cast is plural. Do not collapse the actors into one voice or smooth the strategic dynamics into a single emotional throughline.</rule>
  <rule name="names">Names match the real moment — historical figures, in-world factions, the actual decision-makers. Ground in the record; where it's thin, name the gap rather than invent.</rule>
</paradigm-scene-discipline>`;
...
const PARADIGM_SHAPE_MAP: Record<NarrativeParadigm, string> = {
  ...
  'scenario':     PARADIGM_SCENE_SCENARIO,
};
...
export type GenerateScenesPromptArgs = {
  ...
  /** Whether a `<continuity-basis>` block (committed merges + priors) is present
   *  — decides whether its priority entry is emitted in the hierarchy. */
  hasContinuityBasis?: boolean;
  ...
  "scenes": [
    {
      ...
      "threadDeltas": [{"threadId": "T-XX", "logType": "pulse|transition|setup|escalation|payoff|twist|callback|resistance|stall", "updates": [{"outcome": "VERBATIM from this thread's existing outcomes (its <market>); a label not already listed must also appear in addOutcomes", "evidence": 1.5}], "volumeDelta": 1, "addOutcomes": ["optional — a genuinely new possibility not yet in the stance; once added it may be referenced in updates"], "rationale": "the summary sentence that moved this stance"}],
...
export function buildGenerateScenesPrompt(args: GenerateScenesPromptArgs): string {
  ...
  const priorities = [
    ...
    `  <priority rank="6">WORLD-BUILD FOCUS — recently-introduced entities and latent threads this arc must activate. Bring them on-screen.</priority>`,
    hasPacingSequence ? `  <priority rank="7">PACING SEQUENCE — per-scene mode + force band targets.</priority>` : '',
    `  ${modePriorityEntry(8, "scene-structure")}`,
    `  <priority rank="9">WORK CONTEXT — entities, threads, system knowledge, recent history.</priority>`,
  ].filter(Boolean).join('\n');
...
<summary-discipline>The summary IS the delta budget. Write the summary so every intended delta has a source sentence — every entity-change, rule-surfacing, thread-move, and off-screen-affected party traceable to a sentence. Under-tagging is the dominant failure mode.
  <rule name="no-raw-ids" critical="true">NEVER echo engine identifiers in summary prose. Forbidden tokens: any "PREFIX-NUMBER" form — C-N, L-N, T-N, A-N, S-N, ARC-N, K-N, SYS-N, SYS-GEN-N, etc. The summary is read by downstream prose and plan stages as the authoritative scene brief; an ID slug inside prose ("using SYS-98", "via T-12") leaks engine bookkeeping into the rendered text. Translate every reference to its in-world name or concept:
    ...
    <example bad="The stance on T-12 escalated when Fang Yuan revealed the timeline." good="The question of whether the sect would commit to early defence escalated when Fang Yuan revealed the timeline." />
  ...
  <rule name="no-engine-metadata" critical="true">The summary describes what happened IN-WORLD. NEVER reference engine field names (threadDeltas, worldDeltas, systemDeltas, relationshipDeltas, attributions, attributionEdges, ownershipDeltas, tieDeltas, artifactUsages, events) as bookkeeping inside summary prose. These fields exist SEPARATELY in the JSON; the cross-reference discipline (every delta traces to a summary sentence — see above) lives in your REASONING, not in the summary text.
...
<instructions>
  ...
  <rule name="tag-richly-discipline" hint="Floors and per-tier density bands live in force-standards / deltas; this rule adds scene-shape-specific guidance.">
    ...
    <thread-outcome-continuity critical="true">Each update's outcome MUST be copied VERBATIM from that thread's existing outcomes in its &lt;market&gt; — reuse them so evidence compounds on the same stance (an unlisted label is dropped and the stance stays stuck at even). Open a genuinely new possibility only via addOutcomes, then reference that exact label.</thread-outcome-continuity>
    <profile mode="reflective-pov" hint="Solo-POV scenes, mostly thinking/planning.">POV is STILL the most-changed entity — surface every genuine shift it undergoes (belief/state/goal/capability/secret), plus whatever adjacent entities accrue (location witnessed, artifact handled, off-screen party affected). Read the prose, not a count. One POV delta in a reflective scene is almost always under-tagged.</profile>
...
${sharedRulesBlock}
  ...
  <closing-step name="attributions-and-edges" critical="true" hint="Emit LAST — after every other field. Synthesise from the scene you just wrote.">
    <attributions>Existing ids the scene structurally leans on (C/L/A/T/SYS), one per load-bearing reference; skip merely-on-screen. 0–12 typical. Each id MUST be an exact id already present in the world-state context above (canonical form, e.g. SYS-123, C-12, T-5) — never fabricate or guess one. If the exact id isn't in context, omit it.</attributions>

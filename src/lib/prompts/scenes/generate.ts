  ...
  "scenes": [
    {
      ...
      "threadDeltas": [{"threadId": "T-XX", "logType": "pulse|transition|setup|escalation|payoff|twist|callback|resistance|stall", "updates": [{"outcome": "MUST be copied VERBATIM from this thread's existing outcomes in its <market> — reuse them so evidence compounds on the same stance. Only use a label not already listed if you ALSO add it via addOutcomes this scene", "evidence": 1.5}], "volumeDelta": 1, "addOutcomes": ["optional — a genuinely NEW possibility this scene opens that is not already in the stance; once added it may be referenced in updates"], "rationale": "the summary sentence that moved this stance"}],

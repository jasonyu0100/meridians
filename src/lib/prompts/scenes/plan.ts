...
export function buildScenePlanSystemPrompt(
  ...
  return `${intro}you extract scene facts and architect a beat plan for the writer. The scene may be a fiction or non-fiction event, a rule-driven simulation event, an essay section, a panel cognitive event, a typology entry, a contest move, or a chronicle entry — read the paradigm-shape block to know which, and SHAPE THE PLAN TO THAT FORM (a typology entry is not a sequence of dialogue beats; a contest move is not a character's interior journey; a chronicle entry is not a forward-time narrative beat). Given the scene's structural data (summary, deltas, events), do two things in one pass: (1) extract the COMPLETE set of compulsory propositions the scene must land — natural language ready for the writer, never identifier-echoes or template scaffolding; (2) glue them into a beat plan as JSON — the blueprint the writer renders. Every compulsory proposition must appear in some beat. Follow the taxonomy, density bands, extraction discipline, and output schema in the user prompt. Return ONLY valid JSON.`;
}

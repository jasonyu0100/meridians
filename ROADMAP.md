...
- **Value in week one — calibration is the *compounding* payoff, not the wedge.** The headline (calibration) is the slowest, hardest-to-deliver-honestly property: only **confirmed merges over many cycles** make the model truly honest, and there's no shortcut. So the *entry* value cannot depend on it. **Week-one value is transparency + rehearsal:** the room *sees how it actually decides* (divergent stances made visible) and *practises* under uncertainty (the Conviction drill as a facilitation aid) — felt immediately, independent of long-run calibration. Sell the fast, felt thing; let calibration accrue. (Validation: leading indicators — engagement, decisions made in the open, divergence surfaced, confirmed-merge accrual — answer "is it working?" long before calibration does; B4.)
- **Software product, services-led model.** The binding constraint isn't code — it's the **GM**, who facilitates, routes priors, makes the certainty/merge judgment, tunes the conviction economy, and is the **defender of the room's model of reality.** So we sell software *and* the service around it: we supply or train the GM, and **facilitation is a priced service** (manifesto Economics). Plan as services-led, not pure self-serve — B4's land→download→pay is the *software* face; the engagement (we supply/train the GM) is the *services* face, and most early value lands through the latter. Don't architect as if the binding constraint were shippable code.
...
- **Exactly one GM (one master device).** Multi-GM is **unsupported by design** — the database-less, local-first model has no store to reconcile two writers, so a second GM would mean two competing sources of truth. Every room has a single master; everyone else is a non-GM participant (A0 role). Non-GMs reach the room read/write *through* the master (dark via WhatsApp, live via the ngrok tunnel — A6 / B1), never as a second authority. **One writer at a time, not one person forever:** the encrypted `.meridian` (+ the B3 drive) lets the **GM role hand off** to a backup — GM on holiday or leaving doesn't freeze or lose the room, and in the services-led model we can supply continuity. Local-first + encrypted is also the **procurement story**: nothing leaves the org's machine, which eases IT review. (B3 backs up the *file*; the *operation* hands off via the GM role.)
...
- **Cadence is dynamic, not weekly.** "Weekly" is only a default. Real org decisions resolve on wildly different tempos (hours to quarters) and shift by **season**, so cadence is **per-org and may be per-thread** — each thread can resolve on its own clock rather than at one global merge. Don't bake a fixed weekly cycle; the merge fires when a thread (or the room) is ready.
...
- **The fuller model the lifecycle needs** (so "commit or keep pulling" is expressible): a **Perspective PR** `{ state: open | committed | closed, entityRef (character|location|artifact|general), openThreads, comments, participantId? }` and a **war-room session** that records each PR's outcome. **Cardinality is 1 : N** — one entity perspective can have many concurrent open PRs, so the PR, not the entity, is the unit of the loop. **Closed ≠ merged:** a PR can be **closed without merging** (saved for the record, not committed to the trunk). And an **Issue** `{ title, body, entityRef?, author }` is a proposed *future* PR — the backlog. *Issues → PRs → merge*: GitHub all the way down.
- **Build:**
  ...
  - [ ] `Perspective-PR { state: open|committed|closed, openThreads, comments, participantId }` + a war-room session recording each PR's outcome (merged / kept-open / closed-unmerged — all persisted).
  - [ ] `Issue { title, body, entityRef?, author }` — anyone files a future-PR proposal; the room opens it as a PR when ready.
  - [ ] Mobile + identity available to A3 (close-out routing), A6 (dark channel), B1 (live access / roles).
- **Done when:** you can register participants with those four fields, map each to the perspective(s) they play, a perspective exists as an open / committed / closed PR with threads + comments, and an issue can be filed and promoted to a PR.
...
- **Perspective PR lifecycle.** Opening a PR **opens its fate threads on the immediate future** — the questions priors will answer. While the PR is **open**, those threads are *live*: priors (A2) update them, and the threads themselves can be **revised** — add, drop, reword, re-seed outcomes — as the week clarifies what's actually in play (they aren't frozen at open time). A PR also carries **comments** (GM + participant annotations on the stream). A PR may stay **open** across several war rooms; it ends either **merged** (committed to the trunk, A3) or **closed without merging** — saved for the record, just not committed (superseded, didn't pan out, or low-value). Nothing is deleted; closed PRs stay reviewable.
- **Issues — the backlog of future PRs.** Anyone can file an **Issue**: a proposed *future* perspective request — a scenario or vantage worth playing later. The room opens an issue as a PR when it's ready to capture it. (*Issues → PRs → merge* — the GitHub cycle, end to end.)
...
- **Inferred priors (unattended perspectives):** a perspective the room needs but no human fills — **unavailable this round**, or an **external / other-organization party** you'll never seat (adversary, competitor, regulator) — gets its priors **AI-inferred before the next war room**, generated from that perspective's own continuity + open threads. These are **ordinary priors** — same open threads, same stance machinery (A2); only the author differs (engine, not human), and adversary perspectives keep their hostile tilt. Marked as inferred so the GM can tell provenance. **Honest caveat:** an inferred *adversary* is a **stopgap, not an equivalent** — AI is weakest at exactly the hidden-state + deception an adversary seat needs (CONCEPT risks), so the seat you most want filled by inference is the one inference is worst at. A room red-teamed by an inferred seat is **weaker** than one with a hostile human; the GM should know which kind of room they're running.
- **Build:**
...
- **The war room is the merge.** Every perspective's week-long PR (A1) comes in together; the GM (maintainer) reviews them as one overview and **merges only what's certain.** For each open thread the merge criterion is **high-certainty or reality-confirmed** — confirmed by the public record, or by talking to participants — or, where the GM defers to the market, **deterministic** (the highest-logit outcome). Merged outcomes are the resolved fate of the week. **Contested / low-certainty threads don't merge:** they stay open and carry into next week, divergence preserved. Only certainty enters the canonical trunk — but *certainty comes in two grades* (next).
- **Merge provenance — confirmed vs believed.** Tag every merged outcome: **reality-confirmed** (the public record, or what participants verified happened) or **high-certainty-but-unconfirmed** (the room's crystallised conviction — including a deterministic highest-logit call). Keep the two **visually distinct, permanently.** **Enforce it in software, not by GM discipline:** the resolver **may not read a *believed* node as fact** — believed merges are available as the room's belief, but only **confirmed** nodes are ground truth the resolver treats as real. A believed merge is **never** silently promoted to confirmed. Under time pressure a human maintainer *will* let the trunk drift to self-confirming belief unless the code refuses — so this is a **hard constraint**, not a tag. (This is the narrative-coherence contamination CONCEPT warns about; it extends the inferred-priors provenance discipline, A1, and matters more here.)
...
- **Commit, keep pulling, or close — per perspective.** A war room doesn't force-merge everything. The GM decides **per PR**: **commit (merge)**, **keep pulling (leave open)**, or **close without merging** (saved, not committed). A perspective with nothing significant this cycle isn't merged — its PR persists, comments and open threads intact, into the next war room. Because each perspective is individualised there are **no merge conflicts** — every decision is a standalone per-PR call, and nothing is deleted.
...
- **Same resolver, different *trigger*.** The CRG resolver is shared, but what *triggers* resolution differs by clock — don't collapse them: **Capture** resolves by **certainty / confirmation** (calibration — read the leaned belief, confirm against reality; the deterministic highest-logit merge is a *capture* move, reading where the logits ended up); **Rehearsal / Conviction** resolves by **conviction-forcing** (agency — what a player *paid* to make happen). Put plainly: **Capture measures what reality *is*** (belief, calibrated over time, confirmed at merge); **Conviction plays how committed actors make reality *evolve*** (agency in life). One resolver, two triggers — slow calibration and fast forcing legitimately differ, and that's fine.
- **Single vs multi-merge; causal vs general.** The GM tunes *how much* and *how* the narrative updates. **Granularity:** merge **one thread (discrete)** or **combine several threads / perspectives into one update (nuanced)**. **Method:** **CRG** when the threads *interact causally* (the combined `fate` nodes are reconciled into one continuation), or a **general non-CRG update** (a direct world / scene-arc write) when causality isn't the point. So the GM picks exactly which parts of reality move, and whether they move as a reasoned causal whole or as plain facts.
...
- **Build:**
  ...
  - [ ] Merge controls: **single vs multi-thread** merge, and **CRG vs general (non-CRG)** update — GM picks per merge.
...
- **Depends on:** A3 (the **shared** causal resolver) + the existing Scenarios/Compass. **Scope it honestly:** the *resolver* is shared, but *Conviction is substantial net-new software* — simultaneous multi-seat commit, the budget-and-decay economy, face-up/down hidden information + reveal, differentiated hand-dealing, real-time concurrent state. Capture and the game **share an engine, not a system.** This is the **riskiest** part of the spec (R7 fenced) — don't under-scope it as "mostly reuse."
...
- **Build:**
  - [ ] A mobile-supported page exposing the mobile-relevant features: **contextual chat** (same as the sidebar chat), **prior capture** (per-perspective, A1), and the **Conviction gameplay controller** (A4 — your perspective feed, hand, and GM-gated history; a controller into the game, not the full console). The same page serves capture and play; the console keeps the full interface.
  - [ ] A role / permission split — GM elevated; each non-GM scoped to their own perspective's feed, capture, chat, and history (history depth GM-configurable).
...
- **Goal:** distribute and monetize — **services-led** (we supply/train the GM; facilitation is priced), with the app as the substrate.
- **Build:**
  - [ ] **Landing page** — present the product; download the Electron build / book an engagement.
  - [ ] **Subscription + service billing** tied to accounts (through B3), following the unbundled pricing in the manifesto Economics section (setup / facilitation / software).
  - [ ] **Validation surface — "is it working?"** the first question the economic buyer asks, and the one calibration can't answer fast enough. Surface **leading indicators**: engagement + cadence kept, decisions made in the open, divergence surfaced per cycle, confirmed-merge accrual (the model maturing). Plus the **lagging** proof: confirmed-merge calibration over time. So a buyer can tell it's working long before long-run calibration lands.
- **Done when:** a buyer can land, start an engagement (or download), pay, and **read a working-signal** from week one.
